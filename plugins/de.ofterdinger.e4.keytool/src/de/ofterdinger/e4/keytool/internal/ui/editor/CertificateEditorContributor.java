...
public class CertificateEditorContributor extends MultiPageEditorActionBarContributor {
	...
	public void setActiveEditor(IEditorPart activeEditorPart) {
		if (activeEditorPart != null) {
			CompleteCertificate completeCertificate = ((CertificateEditor) activeEditorPart).getCompleteCertificate();
	...
	public final void setActivePage(IEditorPart part) {
		...
		CompleteCertificate completeCertificate = ((CertificateEditor) this.activeEditorPart).getCompleteCertificate();
		this.exportAction.setCompleteCertificate(completeCertificate);
	}

	private void createActions() {
		this.exportAction = new ExportCertificateAction(null);
		this.exportAction.setImageDescriptor(ImageKeys.getImageDescriptor(ImageKeys.EXPORT_CERTIFICATE));
		this.exportAction.setId("Keytool.actions.exportAction"); //$NON-NLS-1$
		PlatformUI.getWorkbench().getHelpSystem().setHelp(this.exportAction, TextConstants.KEYSTORE_VIEW_HELP_ID);
	}
