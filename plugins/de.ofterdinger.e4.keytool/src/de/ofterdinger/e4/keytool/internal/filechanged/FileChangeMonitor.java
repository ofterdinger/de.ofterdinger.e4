...
public class FileChangeMonitor {
	...
	private static final String CAN_ONLY_INCLUDE_SUBDIRS_FOR_DIRECTORIES = "Can only include subdirs for directories "; //$NON-NLS-1$
	private static final String IS_A_FILE = " is a file."; //$NON-NLS-1$
	...
	public final void addSource(File file, boolean includeSubDirs, FileFilter filter) {
		if (file.isFile()) {
			if (includeSubDirs) {
				throw new IllegalArgumentException(CAN_ONLY_INCLUDE_SUBDIRS_FOR_DIRECTORIES + file.getAbsolutePath() + IS_A_FILE);
			}
			if (filter != null) {
				throw new IllegalArgumentException("FileFilter only allowed for directories " + file.getAbsolutePath() + IS_A_FILE); //$NON-NLS-1$
			}
	...
	public final void addSource(File file, boolean includeSubDirs, FilenameFilter filenameFilter) {
		if (file.isFile()) {
			...
			if (filenameFilter != null) {
				throw new IllegalArgumentException("FilenameFilter only allowed for directories " + file.getAbsolutePath() + IS_A_FILE); //$NON-NLS-1$
			}
			this.monitors.add(new FileMonitor(file));
		} else {
	...
	public final void startMonitor() {
		...
		this.timer.schedule(new TimerTaskClone() {

			@Override
			public void run() {
				FileChangeMonitor.this.monitor();
			}
		}, this.scanRate, this.scanRate);
	}
	...
	private final void firePendingFileChangedEvents() {
		if (!this.pendingUpdates.isEmpty()) {
			for (IFileChangeListener listener : this.listeners) {
				listener.fileChanged(this.pendingUpdates.toArray(new FileChangedEvent[this.pendingUpdates.size()]));
			}
			this.pendingUpdates.clear();
		}
	}
	...
	class DirMonitor implements SubMonitor {
		...
		public DirMonitor(File theDir, FilenameFilter theFilenameFilter, FileFilter theFileFilter, boolean theIncludeSubDirs) {
			...
			if (this.dir.exists() && !this.dir.isDirectory()) {
				throw new IllegalArgumentException("DirMonitor can only run on directories " + theDir.getAbsolutePath()); //$NON-NLS-1$
			}
		}
		...
		public void run() {
			...
			while (i < files.length) {
				...
				if ((!files[i].isDirectory() || this.includeSubDirs) && this.fileToMonitorMap.get(files[i]) == null) {
		...
		private File[] getFiles() {
			if (this.filenameFilter != null)
				return this.dir.listFiles(this.filenameFilter);
	...
	class FileMonitor implements SubMonitor {
		...
		public FileMonitor(File f) {
			...
			if (this.file.isDirectory()) {
				throw new IllegalArgumentException("FileMonitor can only monitor files."); //$NON-NLS-1$
			}
		}
