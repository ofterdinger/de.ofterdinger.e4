...
public class NewCertificateWizard extends Wizard implements INewWizard {
		...
		IRunnableWithProgress op = monitor -> {
			try {
				if (AbstractWizardPage.CREATE_A_NEW_KEYSTORE_TEXT.equals(filename)) {
					NewCertificateWizard.this.insertIntoNewKeystore(alias, filenamePassword, newFilename, password, keystoreType, person,
							monitor, notBefore, notAfter);
				} else {
					NewCertificateWizard.this.insertIntoKeystore(alias, password, filename, person, monitor, notBefore, notAfter);
	...
	private void insertIntoKeystore(String alias, String password, String filename, CertificatePerson person, IProgressMonitor monitor,
			Date notBefore, Date notAfter) {
		monitor.beginTask("Creating " + alias + " certificate", 3); //$NON-NLS-1$ //$NON-NLS-2$
		CompleteCertificate completeCertificate = CertTools.createCertificate(person, notBefore, notAfter);
		...
		try (FileOutputStream fos = new FileOutputStream(keystoreFile.getKeystorefilename())) {
			...
			keystore.setKeyEntry(alias, completeCertificate.getPrivateKey(), password.toCharArray(), chain);
			keystore.store(fos, keystoreFile.getPassword().toCharArray());
		} catch (Exception e) {
			KeytoolPlugin.showError("Error inserting certificate into keystore!", getContainer().getShell()); //$NON-NLS-1$
			KeytoolPlugin.getDefault().getLog().log(new Status(ERROR, PLUGIN_ID, e.getMessage(), e));
		}
	...
	private void insertIntoNewKeystore(String alias, String filePassword, String filename, String password, KeystoreType keystoreType,
			CertificatePerson person, IProgressMonitor monitor, Date notBefore, Date notAfter) {
		monitor.beginTask("Creating " + alias + " certificate", NO_OF_TASKS); //$NON-NLS-1$ //$NON-NLS-2$
		CompleteCertificate completeCertificate = CertTools.createCertificate(person, notBefore, notAfter);
		completeCertificate.setAlias(alias);
		completeCertificate.setPassword(password);
		try {
			CertTools.addCertificateToNewKeystore(completeCertificate, alias, password, filePassword, filename, keystoreType);
			monitor.worked(1);
		} catch (Exception e) {
			KeytoolPlugin.showMessage("Error", "Error creating keystore!", getContainer().getShell()); //$NON-NLS-1$ //$NON-NLS-2$
			KeytoolPlugin.getDefault().getLog().log(new Status(ERROR, PLUGIN_ID, e.getMessage(), e));
		}
	...
	private void openFileForEditing(String alias, IProgressMonitor monitor, final CompleteCertificate completeCertificate) {
		...
		getShell().getDisplay().asyncExec(() -> {
			...
			try {
				...
				KeytoolPlugin.showError("Error showing certificate!", NewCertificateWizard.this.getContainer().getShell()); //$NON-NLS-1$
				KeytoolPlugin.getDefault().getLog().log(new Status(ERROR, PLUGIN_ID, e.getMessage(), e));
			}
		});
	}
