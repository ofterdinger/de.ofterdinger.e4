...
public class ExportCertificateWizard extends Wizard implements INewWizard {
	...
	public boolean performFinish() {
		...
		if (exportPrivateKey) {
			try {
				this.completeCertificate = CertTools.getCompleteCertificate(this.completeCertificate.getKeystoreFile(),
						this.completeCertificate.getAlias(), this.expPage.getCertificatePassword());
			} catch (Exception e) {
			...
			try {
				try {
					if (exportPrivateKey) {
						...
						ExportCertificateWizard.this.createCertificateFile(ExportCertificateWizard.this.completeCertificate.getAlias(),
								filename, monitor);
					}
				} catch (Exception e) {
					KeytoolPlugin.getDefault().getLog().log(new Status(ERROR, PLUGIN_ID, e.getMessage(), e));
					throw new InvocationTargetException(new RuntimeException("Error creating certificate!")); //$NON-NLS-1$
				}
			} finally {
				monitor.done();
			}
	...
	private void createCertificateFileWithPrivateKey(String alias, String filename, IProgressMonitor monitor) {
		monitor.beginTask("Saving " + alias + " certificate", NO_OF_TASKS); //$NON-NLS-1$ //$NON-NLS-2$
		monitor.worked(1);
