...
class TimerClone {
	private static final String NEGATIVE_DELAY = "Negative delay."; //$NON-NLS-1$
	private static final String NON_POSITIVE_PERIOD = "Non-positive period."; //$NON-NLS-1$
	...
	private void sched(TimerTaskClone task, long time, long period) {
		if (time < 0) {
			throw new IllegalArgumentException("Illegal execution time."); //$NON-NLS-1$
		}
		TaskQueue taskQueue = this.queue;
		synchronized (taskQueue) {
			...
			synchronized (object) {
				if (task.getState() != VIRGIN) {
					throw new IllegalStateException("Task already scheduled or cancelled"); //$NON-NLS-1$
				}
