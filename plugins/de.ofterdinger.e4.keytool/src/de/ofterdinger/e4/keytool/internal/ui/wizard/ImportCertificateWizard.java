...
public class ImportCertificateWizard extends Wizard implements INewWizard {
	...
	public boolean canFinish() {
		return (this.impPage.isPageComplete() && (!this.impPage.isNewKeystore() || this.impPage.isNewKeystore()
				&& this.newKeystorePage.getFilename().length() > 0 && this.newKeystorePage.getPassword().length() > 0));
	}
	...
	public boolean performFinish() {
		...
		final String keystoreFilename = isNewKeystore ? this.newKeystorePage.getFilename() : this.impPage.getKeystoreFilename();
		...
		IRunnableWithProgress op = new IRunnableWithProgress() {
			...
			private void importCertificateFile(IProgressMonitor monitor) throws UnrecoverableKeyException, KeyStoreException,
					NoSuchProviderException, NoSuchAlgorithmException, CertificateException, InvalidKeySpecException, IOException {
			...
			private void insertIntoNewKeystore(CompleteCertificate completeCertificate, IProgressMonitor monitor)
					throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException {
				CertTools.addCertificateToNewKeystore(completeCertificate, alias, password, keystorePassword, filename, keystoreType);
				monitor.worked(1);
