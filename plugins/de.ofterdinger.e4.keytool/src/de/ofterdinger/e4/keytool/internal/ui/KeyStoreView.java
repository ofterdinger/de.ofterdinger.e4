...
public class KeyStoreView extends AbstractNavigationView implements IFileChangeListener {
	...
	public final void createPartControl(Composite parent) {
		...
		PlatformUI.getWorkbench().getHelpSystem().setHelp(getViewer().getControl(), KEYSTORE_VIEW_HELP_ID);
		loadAutostartupFile();
	...
	public final void dispose() {
		...
		KeytoolPlugin.getDefault().getPreferenceStore().removePropertyChangeListener(this.preferenceListener);
		super.dispose();
	}
	...
	public final void init(IViewSite site) throws PartInitException {
		super.init(site);
		KeytoolPlugin.getDefault().getPreferenceStore().addPropertyChangeListener(this.preferenceListener);
	}
	...
	protected final Object createModel() {
		try {
			...
			KeytoolPlugin.showError("Error creating model for tree!", getSite().getShell()); //$NON-NLS-1$
	...
	private void loadKeystoreFile(String keystoreFilename, KeystoreType keystoreType, String keystorePassword) {
		monitor.stopMonitor();
		try {
			KeystoreFile keystoreFile = CertTools.loadKeystoreFile(keystoreFilename, keystoreType, keystorePassword);
	...
	private void makeActions(IWorkbenchWindow window) {
		this.openKeystoreAction = new Action() {
		...
		this.openKeystoreAction.setImageDescriptor(PlatformUI.getWorkbench().getSharedImages().getImageDescriptor("IMG_OBJ_FOLDER")); //$NON-NLS-1$
		...
		this.newCertificateAction = new Action("Create new certificate") { //$NON-NLS-1$
			...
			public void run() {
				...
				if (treeObj != null) {
					treeParent = treeObj instanceof TreeParent ? (TreeParent) treeObj : treeObj.getParent();
				}
		...
		this.doubleClickAction = new Action() {
			...
			public void run() {
				...
				if (treeObj.hasParentKeystore()) {
					try {
						CompleteCertificate certificate = CertTools.getCertificate(treeObj.getParent().getKeystoreFile(),
								treeObj.getName());
						...
						KeytoolPlugin.showError("Error showing certificate!", KeyStoreView.this.getSite().getShell()); //$NON-NLS-1$
						KeytoolPlugin.getDefault().getLog().log(new Status(ERROR, PLUGIN_ID, e.getMessage(), e));
					}
				}
			}
	...
	public static void addMonitorFile(String keystoreFilename, KeystoreType keystoreType, String keystorePassword) {
		if (monitor != null) {
			monitor.postFileChangedEvent(new FileChangedEvent(FILE_ADDED, keystoreFilename, keystorePassword, keystoreType));
			monitor.stopMonitor();
			monitor.startMonitor();
		}
	}

	public static void fireFileChanged(String keystoreFilename, KeystoreType keystoreType, String keystorePassword) {
		if (monitor != null) {
			monitor.postFileChangedEvent(new FileChangedEvent(FILE_UPDATED, keystoreFilename, keystorePassword, keystoreType));
		}
	}
