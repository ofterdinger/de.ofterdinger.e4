...
public class NewCertificatePage extends AbstractWizardPage {
	...
	private static final String LABEL_NAME_OF_YOUR_CITY_OR_LOCALITY = "Name of your City or Locality"; //$NON-NLS-1$
	private static final String LABEL_NAME_OF_YOUR_ORGANIZATIONAL = "Name of your organizational"; //$NON-NLS-1$
	private static final String LABEL_NAME_OF_YOUR_ORGANIZATIONAL_UNIT = "Name of your organizational unit"; //$NON-NLS-1$
	private static final String LABEL_NAME_OF_YOUR_STATE_OR_PROVINCE = "Name of your State or Province"; //$NON-NLS-1$
	private static final String LABEL_PASS = "Password"; //$NON-NLS-1$
	private static final String LABEL_THE_TWO_LETTER_COUNTRY_CODE_FOR_THIS_UNIT = "The two-letter country code for this unit"; //$NON-NLS-1$
	private static final String LABEL_VALID_FROM_DD_MM_YYYY = "Valid from (dd-mm-yyyy)"; //$NON-NLS-1$
	private static final String LABEL_VALID_TO_DD_MM_YYYY = "Valid to (dd-mm-yyyy)"; //$NON-NLS-1$
	private static final String LABEL_YOUR_FIRST_AND_LAST_NAME = "Your first and last name"; //$NON-NLS-1$
	...
	public void createControl(Composite parent) {
		...
		this.firstAndLastnameText = this.makeLine(container, LABEL_YOUR_FIRST_AND_LAST_NAME, StringUtils.EMPTY);
		this.orgUnitText = this.makeLine(container, LABEL_NAME_OF_YOUR_ORGANIZATIONAL_UNIT, StringUtils.EMPTY);
		this.orgText = this.makeLine(container, LABEL_NAME_OF_YOUR_ORGANIZATIONAL, StringUtils.EMPTY);
		this.cityText = this.makeLine(container, LABEL_NAME_OF_YOUR_CITY_OR_LOCALITY, StringUtils.EMPTY);
		this.stateText = this.makeLine(container, LABEL_NAME_OF_YOUR_STATE_OR_PROVINCE, StringUtils.EMPTY);
		Locale defaultLocale = Locale.getDefault();
		this.countryText = this.makeLine(container, LABEL_THE_TWO_LETTER_COUNTRY_CODE_FOR_THIS_UNIT, defaultLocale.getCountry());
		this.fromDateText = this.makeLine(container, LABEL_VALID_FROM_DD_MM_YYYY, getDateFormat().format(new Date()));
		Calendar c = Calendar.getInstance();
		c.add(1, DEFAULT_YEAR_TO_ADD);
		this.toDateText = this.makeLine(container, LABEL_VALID_TO_DD_MM_YYYY, getDateFormat().format(c.getTime()));
	...
	public CertificatePerson getPerson() {
		return new CertificatePerson(this.firstAndLastnameText.getText(), this.orgUnitText.getText(), this.orgText.getText(),
				this.cityText.getText(), this.stateText.getText(), this.countryText.getText());
	}
	...
	protected void dialogChanged() {
		...
		this.checkStatus(this.firstAndLastnameText, "Firstname and lastname must be specified"); //$NON-NLS-1$
		...
		this.checkStatus(this.keystoreCombo, "Specify which keystore to save the certificate in"); //$NON-NLS-1$
	}
