...
public abstract class AbstractShowCertificatePage extends AbstractWizardPage {
	...
	private CertificatePerson person = new CertificatePerson(EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
	...
	protected void addCertificateToPage(Composite composite) {
		...
		if (this.showAll) {
			this.personNameText = this.makeLine(composite, "Your first and last name", this.person.getName()); //$NON-NLS-1$
			this.orgUnitText = this.makeLine(composite, "Name of your organizational unit", this.person.getOrganizationUnit()); //$NON-NLS-1$
			this.orgText = this.makeLine(composite, "Name of your organizational", this.person.getOrganization()); //$NON-NLS-1$
			this.location = this.makeLine(composite, "Name of your City or Locality", this.person.getLocation()); //$NON-NLS-1$
			this.stateText = this.makeLine(composite, "Name of your State or Province", this.person.getState()); //$NON-NLS-1$
			this.countryText = this.makeLine(composite, "The two-letter country code for this unit", this.person.getCountryCode()); //$NON-NLS-1$
		}
		this.validFromText = this.makeLine(composite, "Valid from (dd-mm-yyyy)", this.validFrom); //$NON-NLS-1$
		this.validToText = this.makeLine(composite, "Valid to (dd-mm-yyyy)", this.validTo); //$NON-NLS-1$
	}
}
